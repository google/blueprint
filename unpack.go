	...
	for name, packedProperty := range propertyMap {
		...
		if !packedProperty.unpacked {
			err := &BlueprintError{
				...
				Pos: parseTree.GetSourcePosition(packedProperty.property),
			}
	...
	for _, propertyDef := range propertyDefs {
		...
		if first, present := propertyMap[name]; present {
			...
			errs = append(errs, &BlueprintError{
				...
				Pos: parseTree.GetSourcePosition(propertyDef),
			})
			errs = append(errs, &BlueprintError{
				Err: fmt.Errorf("<-- previous definition here"),
				Pos: parseTree.GetSourcePosition(first.property),
			})
			if len(errs) >= maxErrors {
				return errs
			}
	...
	if !ok {
		return []error{
			fmt.Errorf("%s: can't assign %s value to bool property %q",
				parseTree.GetSourcePosition(property.Value),
